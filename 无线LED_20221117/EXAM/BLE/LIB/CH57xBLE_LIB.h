...
/* BLE library config struct  BLE库配置结构 */
typedef struct tag_ble_config
{
    uint32_t MEMAddr;               // library memory start address  //库内存起始地址
    uint16_t MEMLen;                // library memory size  //库内存大小
    uint32_t SNVAddr;               // SNV flash start address( if NULL,bonding information will not be saved ) //SNV flash起始地址(NULL不保存绑定信息)

    uint16_t SNVBlock;              // SNV flash block size ( default 256 ) //SNV flash块大小(默认256)
    uint8_t SNVNum;                 // SNV flash block number ( default 1 ) //SNV flash块号(默认为1)
    uint8_t BufMaxLen;              // Maximum length (in octets) of the data portion of each HCI data packet( default 27 ) //每个HCI数据包的数据部分的最大长度(以字节为单位)(默认为27)
                                    // ATT_MTU = BufMaxLen-4,Range[23,ATT_MAX_MTU_SIZE]
    uint8_t BufNumber;              // Maximum number of sent and received packages cached by the controller( default 5 ) //控制器缓存的最大发送和接收包数(默认为5)

                                    // Must be greater than the number of connections. //必须大于连接数。
    uint8_t TxNumEvent;             // Maximum number of TX data in a connection event ( default 1 ) //连接事件中发送数据的最大数量(默认为1)
    uint8_t TxPower;                // Maximum transmit power level( default LL_TX_POWEER_0_DBM(0dBm) ) //最大发射功率(默认ll_tx_power_0_dbm (0dBm))
    uint8_t WakeUpTime;             // Wake up time value in one RTC count //最大发射功率(默认ll_tx_power_0_dbm (0dBm))一次RTC唤醒时间
    uint8_t SelRTCClock;            // system clock select   //系统时钟选择
                                    // bit0-1 00: LSE(32768Hz) 01:LSI(32000Hz) 10:LSI(32768Hz)
                                    // bit7:  1: ble timer(HSE)(must disable sleep) //bit7: 1: ble定时器(HSE)(必须禁用睡眠)
    uint8_t RxNumEvent;             // Maximum number of RX data in a connection event ( default equal to BufNumber ) //连接事件中RX数据的最大数量(默认等于BufNumber)
    uint8_t MacAddr[6];             // MAC address,little-endian( factory default ) //MAC地址，little-endian(出厂默认)
    uint8_t ConnectNumber;          // Connect number,lower two bits are peripheral number,followed by central number //连接号码，低两位是外围设备号码，后面是中心号码
    uint8_t WindowWidening;         // Wait rf start window   //等待射频启动窗口
    uint8_t WaitWindow;             // Wait event arrive window  //等待事件到达窗口
    pfnSrandCB srandCB;             // Register a program that generate a random seed  //注册一个生成随机种子的程序
    pfnIdleCB sleepCB;              // Register a program that set idle  //注册一个设置为idle的程序
    pfnTempSampleCB tsCB;           // Register a program that read the current temperature,determine whether calibration is need//注册一个程序，读取当前温度，确定是否需要校准
    pfnLSICalibrationCB rcCB;       // Register a program that LSI clock calibration //注册一个程序，说明LSI时钟校准
    pfnLibStatusErrorCB staCB;      // Register a program that library status callback  //注册一个带库状态回调的程序
    pfnFlashReadCB readFlashCB;     // Register a program that read flash   //注册一个读取flash的程序
    pfnFlashWriteCB writeFlashCB;   // Register a program that write flash      //注册一个写flash的程序
} bleConfig_t; // Library initialization call BLE_LibInit function  //库初始化调用BLE_LibInit函数
 ...
 */
// TX_MODE call RF_Tx
#define  TX_MODE_TX_FINISH            0x01  //!< basic or auto tx mode sends data successfully //Basic或auto tx模式发送数据成功
                                            //!< if it is in basic mode,it will enter idle state; //如果处于基本模式，则进入空闲状态;
                                            //!< if it is in auto mode,it will wait for receiving //如果处于自动模式，它将等待接收
#define  TX_MODE_TX_FAIL              0x11  //!< basic or auto tx mode fail to send data and enter idle state //Basic或auto tx模式发送数据失败，进入idle状态
#define  TX_MODE_TX_TIMEOUT TX_MODE_TX_FAIL //!< time of data transmission //数据传输时间
#define  TX_MODE_RX_DATA              0x02  //!< auto tx mode receive data(ack) and enter idle state //Auto tx模式接收数据(ack)并进入空闲状态
#define  TX_MODE_RX_TIMEOUT           0x12  //!< auto tx mode receive timeout and enter idle state //Auto tx模式接收超时并进入空闲状态
#define  TX_MODE_HOP_SHUT             0x22  

// RX_MODE call RF_Rx
#define  RX_MODE_RX_DATA              0x03  //!< basic or auto rx mode receive data //基本或自动rx模式接收数据
                                            //!< if it is in basic mode,it will enter idle state; //如果处于基本模式，则进入空闲状态;
                                            //!< if it is in auto mode,it will judge whether the type matches; //如果是auto模式，它会判断类型是否匹配;
                                            //!< if it matches,it will send data(ack),otherwise(rsr=2), it will restart receiving //如果匹配，则发送数据(ack)，否则(rsr=2)，重新开始接收
#define  RX_MODE_TX_FINISH            0x04  //!< auto rx mode sends data(ack) successfully and enters idle state //Auto rx模式成功发送ack (data)，进入idle状态
#define  RX_MODE_TX_FAIL              0x14  //!< auto rx mode fail to send data and enter idle state //Auto rx模式发送数据失败，进入idle状态
#define  RX_MODE_TX_TIMEOUT RX_MODE_TX_FAIL //!< time of data transmission //数据传输时间
...
typedef struct tag_rf_config
{
    uint8_t LLEMode;                  //!< BIT0   0=basic, 1=auto def@LLE_MODE_TYPE
                                      ...
                                      //！<BIT4-5    00-1M  01 2M  10-coded(s8) 11-coded(s2)def@LIE_PHY_TYPE
 ...
 * PUBLIC FUNCTIONS
 */
extern uint32_t tmos_rand( void ); // pseudo-random number  //输出伪随机数
...
/**
 * @brief   start a event immediately //立即启动事件
 *
 ...
 *
 *调用该函数可以启动一个定时器，该定时器将在n个系统时钟时间 内到期。
 *当定时器到期时，调用任务将获得指定的事件，计时器将用超时值重新加载。
 *
...
/**
 * @brief   shut down rf frequency hopping //关闭射频跳频
 *
 *
 * @param   None.
 *
...
/**
 * @brief   single channel mode. //单通道模式。
 *
