...
//看门狗时钟源为IRCH(内部高速时钟)	
	WDCON  = WDTS(WDTS_IRCH) | WDRE(WDRE_reset);   	//设置看门狗时钟源为IRCH，模式为中断模式
...
//	WDCON  = WDTS(WDTS_XOSCL) | WDRE(WDRE_reset); 	//设置看门狗时钟源为XOSCL，模式为中断模式
//	WDVTHH = 0;										//看门狗中断阈值高八位设置 当前值为1s
//	WDVTHL = 0x0f;									//看门狗中断阈值低八位设置

//看门狗时钟源为IRCL
//	CKCON |= ILCKE;									//使能IRCL
//	WDCON  = WDTS(WDTS_IRCL) | WDRE(WDRE_reset);   	//设置看门狗时钟源为ILCKE，模式为中断模式
//	WDVTHH = 0;										//看门狗中断阈值高八位设置 当前值为1s	
//	WDVTHL = 0x0f;									//看门狗中断阈值低八位设置
