...
#include <intrins.h>
/*********************************************************************************************************************
	本例程设置PWM0、PWM1输出频率为30KHZ、互补并带死区控制的PWM信号
	...
	2、PWMCON、PWMCFG、PWMDIVH、PWMDIVL、PWMDUTH、PWMDUTL等寄存器是
	带索引的寄存器，设置带索引的寄存器前先设置INDEX寄存器，具体操作
	请参考下面的例子
*********************************************************************************************************************/
...
// 	CKSEL = (CKSEL&0xF8) | CKSEL_PLL;			//打开PLL时钟
	...
	P16F = P16_PWM1_SETTING;								//设置P16为PWM引脚功能
...
// 	PWMCON = TIE(0) | ZIE(0) | PIE(0) | NIE(0) | MS(0) | CKS_PLL ;		//设置PWM时钟源为PLL  
	...
	INDEX = PWM_CH1;
	PWMCON = TIE(0) | ZIE(0) | PIE(0) | NIE(0) | MS(0) | MOD(1);			//设置互补模式
	PWMCFG = TOG(0) | 0;
	...
	PWMDUTL	= 7;	
 	...
 	PWMUPD = (1<<PWM_CH0)|(1<<PWM_CH1);									//PWMDIV、PWMDUT更新使能
	while(PWMUPD);																			//等待更新完成
 	PWMEN = (1<<PWM_CH0)|(1<<PWM_CH1);									//PWM0、PWM1使能
